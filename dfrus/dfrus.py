...
def slice_translation(trans_table: Union[Dict[str, str], Iterable[Tuple[str, str]]], bounds) -> Dict[str, str]:
    ...
    if not bounds:
        ...
        if not 0 <= start_index <= end_index < len(trans_table):
            ...
            log.info(
                "Slicing translations (low, mid, high): {}:{}:{}".format(
...
def run(
    path: str,
    dest: str,
    trans_table: Sequence[Tuple[str, str]],
    codepage,
    original_codepage="cp437",
    dict_slice=None,
    debug=False,
    stdout=None,
    stderr=None,
...
@click.option(
    "-n", "--destname", "dest", default="Dwarf Fortress Patched.exe", help="name of the patched DF executable"
)
@click.option(
    "-d",
    "--dict",
