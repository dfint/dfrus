...
seg_prefixes = {
    Prefix.seg_es: Reg.es,
    Prefix.seg_cs: Reg.cs,
    Prefix.seg_ss: Reg.ss,
    Prefix.seg_ds: Reg.ds,
    Prefix.seg_fs: Reg.fs,
    Prefix.seg_gs: Reg.gs,
}
...
def disasm(s: bytes, start_address=0) -> Iterator[DisasmLine]:
    ...
    while i < len(s):
        ...
        if s[i] in op_1byte_nomask_noargs:
            ...
            line = DisasmLine(
                start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(operand1, operand2), prefix=rep_prefix
            )
            ...
            if (si & 0xFE) == mov_rm_imm:
                ...
                line = DisasmLine(
                    ...
                    data=s[j:i],
                    mnemonic=mnemonic,
                    operands=(operand, immediate_operand),
                    prefix=rep_prefix,
                )
            else:
                ...
                line = DisasmLine(
                    start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(operand1, operand), prefix=rep_prefix
                )
            ...
            if op != 7:
                ...
                if op < 2:
                    ...
                    line = DisasmLine(
                        start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(operand,), prefix=rep_prefix
                    )
                elif flag_size:
                    if seg_prefix:
            ...
            line = DisasmLine(
                start_address + j, data=s[j:i], mnemonic="mov", operands=(operand1, operand), prefix=rep_prefix
            )
            ...
            if not dir_flag:
                operand1, operand2 = operand2, operand1
            line = DisasmLine(
                start_address + j, data=s[j:i], mnemonic="mov", operands=(operand1, operand2), prefix=rep_prefix
            )
            ...
            if modrm1 != 1:
                ...
                if modrm1 >= 2:
                    ...
                    line = DisasmLine(
                        start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(operand1,), prefix=rep_prefix
                    )
                elif modrm1 == 0:
                    # test r/m, imm
                    imm_size = 1 << size
                    ...
                    operand = ImmediateValueOperand(immediate)
                    line = DisasmLine(
                        start_address + j, data=s[j:i], mnemonic="test", operands=(operand1, operand), prefix=rep_prefix
                    )
        elif s[i] == 0x0F:
            i += 1
            if s[i] & 0xF0 == x0f_setcc and s[i + 1] & 0xC0 == 0xC0:
                ...
                i += 2
                line = DisasmLine(
                    start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(reg_op,), prefix=rep_prefix
                )
                ...
                line = DisasmLine(
                    start_address + j, data=s[j:i], mnemonic=mnemonic, operands=(dest, src), prefix=rep_prefix
                )
